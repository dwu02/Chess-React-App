...
export default function Square({odd,num,letter,row,id_iterator}: Props) {
    if(num%2===0) {
        if(odd==="true") return <div className = "square white"><div id = {id_iterator} style = {{backgroundImage: `url(${whatAsset(row,letter)})`}} className = 'piece'></div></div>;
        else return <div className = "square dark"><div id = {id_iterator} style = {{backgroundImage: `url(${whatAsset(row,letter)})`}} className = 'piece'></div></div>;
    }
    else {
        if(odd==="true") return <div className = "square dark"><div id = {id_iterator} style = {{backgroundImage: `url(${whatAsset(row,letter)})`}} className = 'piece'></div></div>;
        else return <div className = "square white"><div id = {id_iterator} style = {{backgroundImage: `url(${whatAsset(row,letter)})`}} className = 'piece'></div></div>;
    }
}

// document.getElementById("29").className = "weekend_light";

function whatPieceId(asset: string | undefined) {
...
function whatAsset(row: number,letter: string) {
    // dark pieces
    if(row<3) {
        if(row === 1) {
            if(letter === "A") {
