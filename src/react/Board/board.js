...
let attack_id = -1;
//sounds
let move_sound = new Audio("http://ec2-3-142-151-74.us-east-2.compute.amazonaws.com:3456/public/boop.wav");
let capture_sound = new Audio("http://ec2-3-142-151-74.us-east-2.compute.amazonaws.com:3456/public/capture.mp3");
let checkmate_sound = new Audio("http://ec2-3-142-151-74.us-east-2.compute.amazonaws.com:3456/public/checkmate.wav");
                ...
                if (checkIfValidPieceColor(white, auxiliary_var.style.backgroundImage)) {
                    ...
                    if (determineValidMoves(determinePiece(piece), original_id).has(auxiliary_var.id)) {
                        ...
                        if (white ? !determineCheck(w_king_positions[0]) : !determineCheck(b_king_positions[0])) {
                            ...
                            if (determineCheckMate() === 1){
                                if ((white && match_color === "white") || (!white && match_color === "black")){
                                socket.emit("checkmate", {"username": username, "opponent": opponent, "winner": opponent, "type": "normal"})
                                

                                }
                                else{
                                socket.emit("checkmate", {"username": username, "opponent": opponent, "winner": username, "type": "normal"})

                                }

                            }
                            else if (determineCheckMate() === 2){
                                socket.emit("stalemate", {"current_room": current_room})
                            }

                            socket.emit("relay_move_full", {"piece": piece,"original_id": original_id, "prev_id_zero": prev_id[0], "piece_moved": piece_moved, "prev_id_one": prev_id[1], "room": current_room, "e_id": e.target.id})
            ...
            // move piece to empty square
                    ...
                    if (determineValidMoves(determinePiece(piece), original_id).has(auxiliary_var.id)) {
                        ...
                        if (white ? !determineCheck(w_king_positions[0]) : !determineCheck(b_king_positions[0])) {
                            ...
                            socket.emit("relay_move_empty", {"piece": piece,"original_id": original_id, "prev_id_zero": prev_id[0], "piece_moved": piece_moved, "prev_id_one": prev_id[1], "room": current_room, "e_id": e.target.id})                        // checkBot();
...
socket.on("opponent_move_empty", function(data){
    let change = document.getElementById(data["prev_id_zero"]);
    let auxiliary_var = document.getElementById(data["e_id"]);
    let piece = data["piece"]
    let original_id = data["original_id"];

    if (!(change == null)) change = change.parentElement;
    if (!(change == null)) change.className = data["prev_id_one"];
    auxiliary_var.style.backgroundImage = data["piece_moved"];
...
socket.on("opponent_move_full", function(data){
    let piece = data["piece"]
    let change = document.getElementById(data["prev_id_zero"]);
    let auxiliary_var = document.getElementById(data["e_id"]);
    let original_id = data["original_id"];
...
function getStringID(row, column, piece, capture) {
    ...
    if (!(changeText === null)) {
    changeText.style.color = changeText.innerHTML === "WHITE" ? "black" : "white";
    changeText.innerHTML = changeText.innerHTML === "WHITE" ? "BLACK" : "WHITE";
    }
}
